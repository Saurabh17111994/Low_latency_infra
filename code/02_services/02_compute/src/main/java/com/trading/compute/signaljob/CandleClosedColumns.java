...
public final class CandleClosedColumns {
    ...
    static {
        // P2-127: class-load tripwire — every layout constant must agree on
        // FIELD_COUNT, and index constants must match NAMES positions.
        if (COLUMN_NAMES.size() != FIELD_COUNT
            ...
            throw new IllegalStateException(
                    "CandleClosedColumns drift: COLUMN_NAMES/TYPE_ROOTS/COLUMN_NULLABLE_IN_DDL/ROW_TYPE_INFO"
    ...
    private static void checkIndex(int index, String expected) {
        if (!NAMES[index].equals(expected)) {
            throw new IllegalStateException(
                    "CandleClosedColumns drift: index " + index + " must be '"
