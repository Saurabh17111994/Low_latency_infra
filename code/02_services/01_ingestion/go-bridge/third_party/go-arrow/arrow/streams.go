...
//
// WAVE9-D (P1-046): a read-deadline expiry (idle feed, no ticks for 30s)
// continues the loop — an idle symbol must not require a full reconnect;
// only non-timeout errors (or ctx cancellation) terminate. Text frames
// (e.g. JSON error/keepalive) are ignored: only binary frames reach
// ParseMarketTick. (P1-199.)
func (s *DataStream) ReadTicks(ctx context.Context, onTick func(MarketTick, []byte), onError func(error)) {
	...
	for {
		...
		if err != nil {
			...
			// WAVE9-D (P1-046): idle-feed timeout continues, fatal errors terminate.
		...
		if os.Getenv("ARROW_DEBUG_WS") != "" {
			fmt.Fprintf(os.Stderr, "arrow-debug-ws: read mt=%d len=%d first=%x\n",
				mt, len(payload), payloadPrefix(payload, 8))
		}
...
func (s *OrderStream) ReadUpdates(ctx context.Context, onUpdate func(map[string]any), onError func(error)) {
	...
	for {
		...
		// WAVE9-D (P1-046): idle timeouts continue, fatal errors terminate.
		_ = s.conn.SetReadDeadline(time.Now().Add(defaultStreamReadDeadlineMs * time.Millisecond))
		mt, payload, err := s.conn.ReadMessage()
		if err != nil {
			if ctx.Err() != nil {
