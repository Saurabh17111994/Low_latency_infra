...
pipeline_submit_job() {
  ...
  submit_out="$($COMPOSE exec -T \
    ...
    -e CHECKPOINT_INTERVAL_MS="${CHECKPOINT_INTERVAL_MS:-10000}" -e CHECKPOINT_TIMEOUT_MS="${CHECKPOINT_TIMEOUT_MS:-30000}" -e MAX_CONCURRENT_CHECKPOINTS=1 \
    -e PREVIEW_ENABLED=true -e PREVIEW_INTERVAL_MS="${PREVIEW_INTERVAL_MS:-1000}" \
