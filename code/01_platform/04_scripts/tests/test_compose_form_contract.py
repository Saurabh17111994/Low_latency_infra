...
SITES: list[tuple[str, str, str, str]] = [
    ...
    ("code/01_platform/04_scripts/compose_config_redact.py", "docker compose config` interpolates", "TEXT",
     "docstring: why the render must never be logged verbatim"),
    ("code/01_platform/04_scripts/compose_config_redact.py", "docker compose ... config 2>&1 | compose_config_redact.py",
     "TEXT", "docstring Usage: example of invoking this filter, not an invocation itself"),
    ...
    ("code/01_platform/04_scripts/run-monday-gates.sh", '-f "$COMPOSE_FILE" config', "FLAGS",
     "gate step 2 (P6-525: own log, not STATIC_LOG). The render now pipes through "
     "compose_config_redact.py (2026-09-21 secret-leak fix) before it reaches that log, so this row "
    ...
    ("code/01_platform/04_scripts/tests/test_compose_config_redact.py", "interpolates, so logging its output", "TEXT",
     "docstring: the credential leak this filter prevents (15 soak logs, 2026-09-21)"),
