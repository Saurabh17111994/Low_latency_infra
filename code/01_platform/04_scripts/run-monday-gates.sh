...
if ! (cd "$CODE_DIR" && timeout -k 60 "$JAVA_TIMEOUT_SEC" mvn -o test -pl 02_services/06_execution_gateway \
	"-Dtest=$(surefire_exclude "$DRILL_OWNED_CLASSES_GATEWAY")") >"$GATEWAY_LOG" 2>&1; then
...
# gateway suite does.
if ! (cd "$COMPUTE_DIR" && timeout -k 60 "$JAVA_TIMEOUT_SEC" mvn -o test \
	"-Dtest=$(surefire_exclude "$DRILL_OWNED_CLASSES_COMPUTE")") >"$COMPUTE_LOG" 2>&1; then
