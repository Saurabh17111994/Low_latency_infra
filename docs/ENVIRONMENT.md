...
### FACT-011: Fluss tiers closed segments to R2 and commits the manifest; clients read them back from R2, but R2 is not a tablet restore source (measured on 0.9.1; re-verified on 1.0.0 2026-09-23)
Status: LIVE
Verified: 2026-09-23 - 1.0.0 tiering re-verified live: a fresh datalake.enabled=true table tiered 8 lake objects / 136,222 B into a FRESH R2 prefix with 0 tablet tiering errors; raw_table_1 and its stale lake path untouched (no archiving). Evidence: logs/soak/tiering-1.0-verify-20260923T054824Z/ (runner code/01_platform/04_scripts/tiering-1.0-verify.sh). Caveat: the run needed window-only tablet overrides (task interval 1m, log.segment.file-size 1mb), both reverted - a green run does not extend the certificate.
Verified: 2026-09-22 - image bumped 0.9.1-incubating to 1.0.0 (5feea717) and fluss-fs-s3 1.0.0 is AWS SDK v2 only (6,659 v2 entries, 0 v1; S3A timeout keys survive, verified by inspection). The tiering BEHAVIOUR below is NOT re-verified on 1.0.0: no live R2 tiering trial has run since the bump, and the cert stack currently runs remote.log.task-interval-duration=0s (FACT-018). This row stays LIVE as a 0.9.1 measurement, NOT as a 1.0.0 claim - re-run tiering-smoke.sh with the interval restored before quoting it for 1.0.0.
...
Recheck when: Fluss image bump, or a change to remote.data.dir (the 1.0.0 image bump fired this recheck - answered by the 2026-09-23 line)
