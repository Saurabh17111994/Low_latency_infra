...
> **Note (2026-09-23).** The table below is the 2026-08-24 offline snapshot and is kept as
> history. The Fluss rows (VM-JAVA-001, VM-FLINK-SRV-003, VM-FLUSS-SRV-005, VM-FLUSS-CLI-006,
> VM-FLUSS-CONN-007, VM-IMAGES-012) were **re-recorded against Fluss 1.0.0 on 2026-09-23** in
> `code/01_platform/04_scripts/version_matrix.yaml` (Stage 4 / T4.1 of the upgrade plan). The pin
> digests of record are `FLUSS_CLIENT_JAR_SHA256=a73101b2...` and
> `FLUSS_FLINK_CONNECTOR_JAR_SHA256=8cad6d13...`; the `6921994a` / `5dddeb` values quoted in the
> rows below are 0.9.1-era digests and no longer match `versions.pin`.
...
| VM-FLINK-SRV-003 / VM-FLINK-API-004 Flink 2.2.1 | PARTIALLY (pin verified) | `code/pom.xml` flink 2.2.1 managed deps compile green; `docker compose config` parses `flink-jobmanager:2.2.1` via fluss-flink-2.2 connector `5dddeb...` (0.9.1-era digest, superseded) SHA, `flink-streaming-java` `bb41cde...` | Live `COMPAT-FLINK-001` checkpoint/restore/rescale + `SIG-HARNESS-003/005` on Docker Flink 2.2.1 |
...
| VM-FLUSS-CLI-006 Fluss client 0.9.1 | COMPATIBLE (2026-08-24) | `versions.pin` `FLUSS_CLIENT_JAR_SHA256=6921994a2067...` (0.9.1-era digest, superseded — see the note above) official client jar, `CompatFlussIntegrationTest` compile vs 0.9.1-incubating 100/1000 appends 0 loss | Routing/bucket-key stress + connector checkpoint closed 2026-08-24; replication = server/topology boundary |
...
| VM-IMAGES-012 Base images | UNKNOWN (2026-08-24 partial — digest+SBOM verified; vuln-policy unverified) | `versions.pin` digests: `FLUSS_CLIENT 6921994a`, `FLUSS_FLINK 5dddeb` (0.9.1-era, superseded — see the note above), `FLINK_STREAMING bb41cde` (still current: Flink remained 2.2.1), `docker-compose.yml` `${FLUSS_IMAGE:?set ...}` digest-required, `pin-check.sh` no SNAPSHOT | 12/12 digests verified + SBOM attestations (4/5 images) + OS-layer trivy counts disclosed (`logs/version-matrix/images-012-20260824T171559Z.md`); library-layer scan-unavailable, SEC-IMAGE-001 threshold TO_BE_VERIFIED |
...
|---|---|---|
| VM-JAVA-001 | `COMPATIBLE` (`17.0.19` exact) | `docker run --rm flink:2.2.1-scala_2.12-java17 java -version` → Temurin 17.0.19+10; host JVM 17.0.19 identical; full module suites green on it (common 489 / ingestion 285 / compute 419) |
