...
| 4 | Multi-node pre-deploy validation | `stack_selfcheck.sh` refuses to run when the swarm has more than one node ("single-host mimic, not a cluster"), so nothing validates a real cluster's stack before `docker stack deploy`. Either parameterise that script for a real cluster or add a cluster validator covering the 15 `required_vars`, `docker stack config`, placement labels and node roles. | Validator green against a multi-node swarm, with its failure modes asserted offline | VM guide `S7` |
