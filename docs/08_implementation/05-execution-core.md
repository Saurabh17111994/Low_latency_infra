...
| Nautilus -> Fluss 9 sinks | `PostbackCorrelator` 3-step broker->client->reconciliation + bijective; `PositionProjector` 12/12 + Rust `projection` 7/7 oversell->UNKNOWN; `DurableFlags` OFF bit-identical and now flag-selecting (`DurableClients::open_for_service` + file-backed `durable_file` stores, CHG-135; `durable` 24 / `durable_file` 6 tests) | DDL `APPLY=1` + Java `FlussProjectionWriterIT` SKIPPED until `FLUSS_BOOTSTRAP` live single-VM |
...
| `T5` durable gate/attempt/fencing | **PARTIAL — Rust durable stores landed (D1/D2), live caller still absent** | Pure-JVM `GateTransitionValidator`, `InMemoryAttemptStore`, audit/hash-chain, identity tests; Rust `FileAttemptStore`/`FileGateStore` behind the durable flags with `try_claim` and create-on-halt, restart recovery covered by reopen tests (CHG-135) | No durable gateway-backed state, and no *live* Rust caller: `ExecutionGate` is still constructed only by tests, so pre-call persistence, fencing, approvals, and crash-window enforcement in the running service remain Workstream D |
