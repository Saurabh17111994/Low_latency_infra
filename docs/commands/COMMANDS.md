...
| Action | Command | What it does |
|---|---|---|
| Full Monday gate | `make gate` | The big verification gate (`run-monday-gates.sh`), **19 steps**: static/compose/python/entrypoint/Go/E2E-build/docker-smoke/image-staleness/java+drills/doc-audit/DDL-smoke/schema-perf/SIGTERM-drain plus the execution-gateway, compute and Rust module suites. Prints "N/M verified" and counts a skipped step as unverified, not green. Step 3 itemizes its skips — it runs the unittest suites verbosely and **fails the step** when the summary claims a skip the log cannot name (CHG-220). It also runs the deploy-environment preflight against the tracked template (`deploy_preflight.py --expect dev`, CHG-268), so a stack variable the template cannot supply fails the gate instead of the deploy. Step 8 requires a real build stamp (`--require-stamps`), and the run is remembered against its `(tree, stack)` fingerprint: a re-run of a pair that was already certified says `REPLAYED@<time>`, and `--no-replay` refuses it outright (CHG-125). `--steps N` runs a subset: its SUMMARY ends in `SUBSET RESULT` and it **never** certifies, however many steps it is given |
| Fast loop (not a certificate) | `make gate-fast` | The T0 subset of the offline steps (`make ddl`, static checks, the R2/parity audit suite, pin discipline, fast image staleness, doc↔code truth, the python suites) — ~4 min, no live drills, no DDL apply smoke, no full doc audit. Prints `GATE-FAST RESULT: subset green`. Use it per change; a batch's certificate is still `make gate` |
...
| Import instruments (RETIRED) | `bash code/01_platform/04_scripts/import_instruments.sh` | **Retired (CHG-229)** — refuses with exit 2 and imports nothing. The `instruments` table has one loader: the ingestion service's `InstrumentManifestWriter` (`INSTRUMENT_MANIFEST_PATH`; `docs/08_implementation/03-ingestion.md`, ING-INT-004). The old script emitted a 12-column positional statement for a 14-column KV table whose load contract is enforced in code, not in SQL |
| Evidence ownership check | `make evidence-ownership-check` | Non-root ownership contract gate (C15) |
| Lake health check | `bash code/01_platform/04_scripts/lake-guard.sh` | Daily R2 lake guard. Counts **real data objects** (a non-empty `.parquet` — `_SUCCESS` markers, `.crc` sidecars and zero-byte markers do not count) for yesterday and, after 18:30 IST, for today; also requires the table's newest Iceberg manifest to be ≤48 h old. Lists one prefix per check instead of the whole warehouse, and prints `!! lake-guard FAIL: <reason>` before exiting 1 when the listing itself fails. Knobs: `LAKE_GUARD_CHECK_DAY` (test hook), `LAKE_GUARD_TABLE_PREFIX`, `LAKE_GUARD_MIN_MANIFESTS`, `LAKE_GUARD_MAX_MANIFEST_AGE_H`, `LAKE_GUARD_AFTER`; exit 2 = unusable input (wave 12) |
| Tiering job status / restart | `bash code/01_platform/04_scripts/tiering-start.sh [--status]` | Idempotent Flink tiering-job submit; refuses to submit unless the hadoop-mapreduce-compat jar is in both flink libs and the bad uber jar is absent (CHG-117). Exit 0 = running, 1 = not running/submit failed, 2 = a running job whose restart strategy is neither fixed-delay nor exponential-delay — the two forms that never give up permanently (cancel it, then re-run; CHG-145, updated 2026-09-23: Fluss 1.0.0 sets the tiering job's strategy in its own code, so the old -D pin is ignored) |
...
| Fluss prefix reader | `java -cp $FLUSS_PROBE_CP FlussPrefixReader <table> <tokens\|*> <window_start_ms> <window_end_ms> [bootstrap]` | Raw LOG or KV prefix scan; prints one TSV row per match then `__END__ <n>` on **stdout**. Callers block on that sentinel, so it is printed even when the run fails (`__ERROR__` on stderr, exit 1); exit 2 = unusable input (bad window, no usable token, table without `window_start`). `PROBE_READ_DEADLINE_MS` (default 600000) stops a LOG scan and warns on stderr rather than silently truncating. No in-tree caller — operator tool (wave 13) |
| Perf evidence collector | `bash code/01_platform/04_scripts/perf-evidence-collector.sh <out-dir> [interval-seconds]` | Samples the 7 PERF-AUDIT-001 evidence categories every `interval` (default 30) — Flink subtask busy/backpressure plus records in/out, checkpoint size/duration/count, per-container memory and CPU, per-vertex records-in, and append latency from OpenObserve. Two of the seven (Fluss client internals, Fluss server internals) are documented GAPS with no clean source and are not sampled — into `<out-dir>/snapshots/snap-<n>.json` plus a TSV `summary.tsv`. Throughput is the job **sum** across subtasks (Flink answers `/subtasks/metrics` with one aggregate object, so summing `avg` would report a single subtask's rate) and memory is normalized to bytes, since the raw `docker stats` form mixes MiB and GiB and cannot be compared. Parse/aggregate lives in the companion `perf_evidence_parse.py`. Knobs: `FLINK_JM_CONTAINER`, `JOB_NAME_PATTERN`, `O2_BASE_URL`, `O2_ENV_FILE` (default `01_docker/secrets.env`), `RUN_FOR_S` (stop after N seconds; default 0 = run until INT/TERM). A rerun continues the snapshot index instead of overwriting. Deliberately NOT `set -e` and it keeps going through a failing poll (P6-011), so always read the exit code: 0 = at least one snapshot written, 1 = nothing succeeded so the capture is not usable evidence, 2 = bad usage or config (interval, RUN_FOR_S, an unwritable out-dir, or the companion module missing). No in-tree caller — operator tool |
| Analyze a JFR recording | `python3 code/01_platform/04_scripts/jfr-analyze.py <recording.jfr> --periods NAME=START-END,...` | Buckets sampled stacks by class (`RocksDB` / `FlussClient` / `Netty/SocketIO` / `FlinkMailbox` / `FlinkTimer` / `FlinkCheckpoint` / `FlinkSerde` / `FlinkRuntime` / `Lock/Monitor` / `JVM/Park` / `Alloc/GC` / `Other/Java`) per period and event type, with the top innermost frames, so a stall can be attributed to Flink runtime vs lock contention vs a parked thread. Frames are matched innermost-out, so the leaf frame decides the bucket. Periods are Unix wall-clock instants at **second** resolution (the format `jfr-analyze.py` itself takes) - this is the analyzer's own argument shape and says nothing about tick timestamps, which are milliseconds (DEC-039). Requires the JDK `jfr` tool on PATH; a `jfr` failure exits non-zero through the subprocess check. No in-tree caller — operator tool |
| Fluss KV probe | `java -cp $FLUSS_PROBE_CP FlussKvProbe candle_live 15000 4,7,13 localhost:9123` | CP9→CP10 point lookups; TSV `<epoch_ms>\t<token>\t<window_start>\t<window_end>\t<last_event_time>\t<staleness_ms>`. `window_end`/`last_event_time` are resolved **by name** from the live schema (a reordered table fails loudly). One failing token no longer discards the others. Exit 0 = full sample, 1 = no row for any token, 2 = unusable input, 3 = partial sample (rows are still printed) — `stage-capture.sh` warns per tick (wave 13) |
| Fluss read-lag probe | `java -cp $FLUSS_PROBE_CP FlussReadLagProbe default raw_table_1 localhost:9123` | One TSV sample `<epoch_ms>\t<table>\t<partition_count>\t<bucket_count>\t<log_end_offset>` per tick; all `listOffsets` RPCs are issued before any is collected, so partitions are not read at different times. Exit 0 = complete, 3 = partial (some partitions failed — excluded and named on stderr), 1 = the sample could not be taken (no row printed), 2 = unusable input (wave 13) |
