...
- [ ] **B4.7 Stop putting real credentials in a conversation.** Measured 2026-09-21 with the scanner from
      ...
      **The values are already copied beyond that file — measured 2026-09-21 by value-equality against the
      four real values** (a bounded throwaway scan, because `values_at_rest_scan.py` searched names and
      shapes, not values; T2's value-equality criterion was built the same day as this remediation (`--values-file`, CHG-295), so the check that found these 48 files is now a shipped one rather than a one-off): 48 files outside the source, in
      ...
      **Correction 2026-09-21, measured by T2's value-equality mode once it existed:** the scan behind
      these numbers missed the repository's own copies. `code/01_platform/01_docker/secrets.env` and
      `.env.bak-20260828` hold the password and the TOTP key; both are git-ignored (`:31`, `:32`, `:89`)
      with zero commits touching them, so nothing was published — but "48 files" is an undercount and the
