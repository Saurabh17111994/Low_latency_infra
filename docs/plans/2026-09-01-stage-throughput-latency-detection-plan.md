...
| 2026-09-02 | 20Hz tier (20,480/s = real-broker peak, 1024 instruments): 180s capture | Source 19,851/s (97% of peak), sawtooth; **fingerprint-dedup 74–77% per-subtask occupancy** = the next limiter (aggregate pipeline only ~18% busy — no CPU wall) |
| 2026-09-02 | C2 fix attempt: dedup parallelism 8→16 + RocksDB write-path knobs | p16 **OOM'd the TM heap** at t+71s (304 tasks share ~2.1GB heap; host has ~9GiB free — no headroom) → reverted to p8 + knobs |
| 2026-09-02 | **FLINK_PROPERTIES incident** (found during the p16 work): (1) comments inside the block become garbage config keys (entrypoint never skips `#`); (2) `state.backend` String leaf prefix-collides with `state.backend.rocksdb.*` → ClassCastException crash-loop OR **silent drop of localdir** (RocksDB back on the container overlay = CHG-120 class); (3) `docker compose restart` reuses old container config (slots edit never applied) | Fixed: block = property lines only; `state.backend` leaf removed (job pins backend at job level); RocksDB knobs passed as job-level `-D` flags. **Fail-fast guards G22/G23/G24** (user request): G22 static validator `check_flink_properties.py` + pytest; G23 preflight verifies TM generated config + registered slots ≥ PARALLELISM (with WHY + recreate hint); G24 runner checks RocksDB dirs land on the named volume. 83/83 guards |
| 2026-09-02 | **CHG-122 fully-Docker data path** (user decision: never host-based again): faketool + arrow-bridge + ingestion JVM migrated from host processes to a `loadgen` image (`Dockerfile.loadgen`, compose profile `loadgen`, per-run `docker run` on trading-net; in-network DNS `ws://pipeline-faketool:8899`, `fluss-coordinator:9123` — no NAT hairpin). **G26 policy guard**: fail-fast WITH reason on host data-path processes / stale loadgen containers / TM-network mismatch. G9+G20a tests updated to the new contract. | 95/95 test-pipeline-lib; 20Hz smoke pending |
